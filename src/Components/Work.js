...
function Work({ position, company, location, type, duration }) {
  return (
    <article className="pt-6 pb-5 hover:bg-dark-card/30 transition-colors rounded-lg px-2">
      {/* Top row: position + type badge */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
        <h1 className="text-content md:text-lg  font-medium leading-snug flex-1 text-justify">{position}</h1>
        <span className="bg-greenbg text-green-text text-xs font-medium rounded-3xl px-3 py-1 w-fit whitespace-nowrap self-start">
          {type}
        </span>
      </div>

      {/* Bottom row: company, location, duration */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-3 gap-2">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-1">
          {/* Company */}
          <div className="flex items-center">
            <svg
              className="fill-light-heading shrink-0"
              width="16"
              height="12"
              viewBox="0 0 16 12"
              fill="none"
            ...
            <p className="text-content text-xs md:text-sm font-light pl-1.5">{company}</p>
          </div>
          ...
          <div className="flex items-center">
            ...
            <p className="text-content text-xs md:text-sm font-light pl-1.5">{location}</p>
          </div>
        </div>

        {/* Duration */}
        <p className="text-content text-xs md:text-sm font-light whitespace-nowrap">
          {duration}
        </p>
      </div>

      {/* Short separator line */}
      <div className="w-3/4 border-b border-light-content border-opacity-30 pt-4 mx-auto" />
    </article>
  );
}
